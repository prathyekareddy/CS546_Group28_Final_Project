...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      result = createGroupValidation.checkCreateGroup(
        req.body.groupName,
...
router.post("/search.html", async (req, res) => {
  ...
  if (result.length === 0) {
    res.render("partials/searched-group", {
      layout: null,
      sampleResult: false,
    });
    return;
...
router.route("/sendrequest/:id").post(async (req, res) => {
...
router.route("/groupdetails").get(async (req, res) => {
